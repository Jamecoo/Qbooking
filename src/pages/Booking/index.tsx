...
const Booking = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const response = await axios.get<BookingModel[]>('http://localhost:3030/data');
    ...
    return <Box sx={{ display: 'flex', minHeight: '100vh', justifyContent: 'center' }}>
        <Box sx={{ width: '100%', backgroundColor: '#E8E8E8' }}>
            <Box
                ...
                <IconButton onClick={() => controller.handleButtonClick('/homebooking')}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '40px', marginBottom: '30px' }}>
                <Button
                    onClick={() => controller.handleButtonClick('/billbooking')}
                    type='submit' variant="contained" sx={{ height: '59px', backgroundColor: '#282828', borderRadius: '10px', width: '242px', textTransform: 'none' }}> Booking now </Button>
            </Box>
